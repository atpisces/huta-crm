...
foreach ($rResult as $aRow) {
	...
	foreach ($pdf_custom_fields as $field) {
		$value = get_custom_field_value($aRow['staffid'], $field['id'], 'staff');
		if ($value == '') {
		  continue;
		}
		if($field['name'] == "Iqama Expiry"){
			$iqama_expiry = $value;
		}
		if($field['name'] == "Passport Expiry"){
			$passport_expiry = $value;
		}
		if($field['name'] == "Medical Insurance Expiry"){
			$insurance_expiry = $value;
		}
	}

	$date1 = date("d-m-Y"); 
	$date2 = date("d-m-Y",strtotime($iqama_expiry));
	$diff = abs(strtotime($date2) - strtotime($date1)); 

	$iyears   = floor($diff / (365*60*60*24)); 
	$months  = floor(($diff - $years * 365*60*60*24) / (30*60*60*24)); 
	$days    = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
	if($iqama_expiry == ""){
		$row[] = "";
	} else if($iyears <= 1){
    	// $row[] = $iqama_expiry." : ".$iyears;
    	$row[] = '<span class="label label-danger"> Expiring in: <b>'.$iyears.'years '.$months.'months '.$days.'days.</b> </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'iqama\');">+ Extend Iqama</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewIqamaHistoryModal'.$aRow['staffid'].'">View History</button>';
	} else {
    	$row[] = '<span class="label label-success"> Iqama is Valid </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'iqama\');">+ Extend Iqama</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewIqamaHistoryModal'.$aRow['staffid'].'">View History</button>';
	}

	$date1 = date("d-m-Y"); 
	$date2 = date("d-m-Y",strtotime($passport_expiry));
	$diff = abs(strtotime($date2) - strtotime($date1)); 
	...
	if($passport_expiry == ""){
    	...
    	// $row[] = $iqama_expiry." : ".$iyears;
    	$row[] = '<span class="label label-danger"> Expiring in: <b>'.$iyears.'years '.$months.'months '.$days.'days.</b> </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'passport\');">+ Extend Passport</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewPassportHistoryModal'.$aRow['staffid'].'">View History</button>';
	} else {
    	$row[] = '<span class="label label-success"> Passport is Valid </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'passport\');">+ Extend Passport</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewPassportHistoryModal'.$aRow['staffid'].'">View History</button>';
	}

	$date1 = date("d-m-Y");
	$date2 = date("d-m-Y",strtotime($insurance_expiry));
	$diff = abs(strtotime($date2) - strtotime($date1)); 
	...
	if($insurance_expiry == ""){
    	...
    	// $row[] = $iqama_expiry." : ".$iyears;
    	$row[] = '<span class="label label-danger"> Expiring in: <b>'.$iyears.'years '.$months.'months '.$days.'days.</b> </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'medical\');">+ Extend Medical Insurance</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewMedicalInsuranceHistoryModal'.$aRow['staffid'].'">View History</button>';
	} else {
    	$row[] = '<span class="label label-success"> Insurance is Valid </span><br><br>
    		<button type="button" class="btn-info btn-sm" data-toggle="modal" data-target="#extendDateModal" onclick="extendDate('.$aRow['staffid'].',\'medical\');">+ Extend Medical Insurance</button>
    		<button type="button" class="btn-warning btn-sm" style="margin-top: 3px;" data-toggle="modal" data-target="#viewMedicalInsuranceHistoryModal'.$aRow['staffid'].'">View History</button>';
	}
