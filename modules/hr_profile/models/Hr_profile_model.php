...
class Hr_profile_model extends App_Model
{
	...
	public function save_extend_status_date($data){

		$save = $this->db->insert(db_prefix().'staff_status_records', $data);
		$last_id = $this->db->insert_id();
		if($last_id){
			$extend_duration = explode(",",$data["extend_duration"]);
			$iqama_expiry = ""; $passport_expiry = ""; $insurance_expiry = "";
			$pdf_custom_fields = get_custom_fields('staff', array('show_on_table' => 1));
			foreach ($pdf_custom_fields as $field) {
				$value = get_custom_field_value($data['staffid'], $field['id'], 'staff');
				if ($value == '') {
				  continue;
				}
				if($field['name'] == "Iqama Expiry" && $data["statustype"] == "iqama"){
					// echo "<br><br>here:<br><br>";
					$iqama_expiry = $value;
					$date = strtotime($iqama_expiry);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[0]." year",$date));
					$date = strtotime($newexpirydate);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[1]." months",$date));
					$date = strtotime($newexpirydate);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[2]." days",$date));

       				$date_date = array('old_date' => $iqama_expiry, 'new_date' => $newexpirydate);
					$this->db->where('statusid', $last_id);
					$this->db->update(db_prefix() . 'staff_status_records', $date_date);

					$update_values = array('value' => $newexpirydate);
					$this->db->where('relid', $data['staffid']);
					$this->db->where('fieldid', $field['id']);
					$this->db->update(db_prefix() . 'customfieldsvalues', $update_values);
					// echo $this->db->last_query();
					if ($this->db->affected_rows() > 0) {
						return true;
					}
				}
				if($field['name'] == "Passport Expiry" && $data["statustype"] == "passport"){
					$passport_expiry = $value;
					$date = strtotime($passport_expiry);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[0]." year",$date));
					$date = strtotime($newexpirydate);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[1]." months",$date));
					$date = strtotime($newexpirydate);
       				...
       				$date_date = array('old_date' => $passport_expiry, 'new_date' => $newexpirydate);
					$this->db->where('statusid', $last_id);
					$this->db->update(db_prefix() . 'staff_status_records', $date_date);
				...
				if($field['name'] == "Medical Insurance Expiry" && $data["statustype"] == "medical"){
					$insurance_expiry = $value;
					$date = strtotime($insurance_expiry);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[0]." year",$date));
					$date = strtotime($newexpirydate);
       				$newexpirydate = date('d-m-Y',strtotime("+".$extend_duration[1]." months",$date));
					$date = strtotime($newexpirydate);
       				...
       				$date_date = array('old_date' => $insurance_expiry, 'new_date' => $newexpirydate);
					$this->db->where('statusid', $last_id);
					$this->db->update(db_prefix() . 'staff_status_records', $date_date);
	...
	public function get_staff_list($staffid = ''){
		if($staffid == ''){
			return $this->db->query("SELECT * FROM tblstaff")->result_array();
		} else {
			return $this->db->query("SELECT * FROM tblstaff WHERE staffid = '$staffid'")->result_array();
		}
	}

	public function get_statuses_records($staffid = '', $statustype = '', $fromdate = '', $todate = ''){
		if($staffid == '' && $statustype == ''){
			$query = "SELECT * FROM tblstaff_status_records";
			if($fromdate != "" && $todate != ""){
				$query .= " WHERE CAST(added_datetime AS DATE) >= '".date("Y-m-d",strtotime($fromdate))."' AND CAST(added_datetime AS DATE) <= '".date("Y-m-d",strtotime($todate))."'";
			}
			return $this->db->query($query)->result_array();
		} else if($staffid != '' && $statustype == ''){
			$query = "SELECT * FROM tblstaff_status_records WHERE staffid = '$staffid'";
			if($fromdate != "" && $todate != ""){
				$query .= " AND CAST(added_datetime AS DATE) >= '".date("Y-m-d",strtotime($fromdate))."' AND CAST(added_datetime AS DATE) <= '".date("Y-m-d",strtotime($todate))."'";
			}
			return $this->db->query($query)->result_array();
		} else if($staffid == '' && $statustype != ''){
			$query = "SELECT * FROM tblstaff_status_records WHERE statustype = '$statustype'";
			if($fromdate != "" && $todate != ""){
				$query .= " AND CAST(added_datetime AS DATE) >= '".date("Y-m-d",strtotime($fromdate))."' AND CAST(added_datetime AS DATE) <= '".date("Y-m-d",strtotime($todate))."'";
			}
			return $this->db->query($query)->result_array();
		} else if($staffid != '' && $statustype != ''){
			$query = "SELECT * FROM tblstaff_status_records WHERE staffid = '$staffid' AND statustype = '$statustype'";
			if($fromdate != "" && $todate != ""){
				$query .= " AND CAST(added_datetime AS DATE) >= '".date("Y-m-d",strtotime($fromdate))."' AND CAST(added_datetime AS DATE) <= '".date("Y-m-d",strtotime($todate))."'";
			}
			return $this->db->query($query)->result_array();
