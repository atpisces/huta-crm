...
class Hr_profile extends AdminController
{
	...
	public function user_status_report(){
		if($this->input->post()){
			...
			$data['status_type'] = $post_data["status_type"];
			$data['from_date'] = $post_data["from_date"];
			$data['to_date'] = $post_data["to_date"];
			$data['staff_list'] = $this->hr_profile_model->get_staff_list($post_data['staff_id']);
			$data['statuses_records'] = $this->hr_profile_model->get_statuses_records($post_data['staff_id'],$post_data["status_type"],$post_data["from_date"],$post_data["to_date"]);
			// echo "<br><br>";
			
			// print_r($data['staff_list']);
			// echo "<br><br>";
			// print_r($data['statuses_records']);
			// exit;
		}
		$data['from_date'] = '';
		$data['to_date'] = '';
		$data['title'] = 'Staff Status Report';
		$data['staff'] = $this->hr_profile_model->get_staff_list();
		$this->load->view('reports/status_report', $data);
	}
	...
	public function add_extend_status_date(){
		if($this->input->post()){
			$data = $this->input->post();
			// print_r($data);
			$data["extend_duration"] = implode(",",$data["extend_duration"]);
			$extend = $this->hr_profile_model->save_extend_status_date($data);
			if($extend){
  ...
  'success' => false
	...
	*/
	public function staff_infor()
	{
		...
		$data['dep_tree'] = json_encode($this->hr_profile_model->get_department_tree());
		$data['staff_dep_tree'] = json_encode($this->hr_profile_model->get_staff_tree());
		$data['staff_list'] = $this->hr_profile_model->get_staff_list();
		$data['statuses_records'] = $this->hr_profile_model->get_statuses_records();
