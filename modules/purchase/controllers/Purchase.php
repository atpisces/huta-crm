...
class purchase extends AdminController
{
    ...
    public function purchase_request(){
        if (!has_permission('purchase_request', '', 'view') && !is_admin()) {
            access_denied('purchase_request');
        }
    ...
    public function pur_request($id = ''){
        if (!has_permission('purchase_request', '', 'create') && !is_admin() && !has_permission('purchase_request', '', 'edit')) {
            access_denied('purchase_request');
        }
    	$this->load->model('departments_model');
    ...
    public function view_pur_order($pur_order){
        ...
        echo json_encode([
            'total' => app_format_money($pur_order->total,$base_currency->symbol),
    ...
    public function purchase_order($id = ''){
        if (!has_permission('purchase_orders', '', 'view') && !is_admin()) {
    ...
    public function contracts(){
        if (!has_permission('purchase_contracts', '', 'view') && !is_admin()) {
            access_denied('_contracts');
        }
        $this->load->model('departments_model');
        $data['departments'] = $this->departments_model->get();
        $this->load->model('projects_model');
    ...
    public function purorder_pdf($id)
    {
        ...
        $data["test"] = "Wahid";
        $pdf->Output('purchase_order.pdf', $type, $data);
    }
...
/**
    ...
    public function items($id = ''){
        if (!has_permission('purchase_items', '', 'view') && !is_admin()) {
            access_denied('items');
        }
                    ...
                    if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                        ...
                        for ($row = 1; $row < count($data); $row++) {
                            ...
                            if (is_null($value_purchase_price) != true && $value_purchase_price != '') {
                                if (!is_numeric($value_purchase_price)) {
                            ...
                            if($flag == 0 && $flag2 == 0){
                                ...
                                $rd['rate']                         = $value_rate;
                                $rd['purchase_price']                         = $value_purchase_price;
