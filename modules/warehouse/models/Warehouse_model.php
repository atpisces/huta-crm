...
class Warehouse_model extends App_Model {
	...
	public function get_pur_request($pur_order) {
		...
		$sql = 'select item_code as commodity_code, ' . db_prefix() . 'items.description, ' . db_prefix() . 'items.unit_id, unit_price, quantity as quantities, ' . db_prefix() . 'pur_order_detail.tax as tax, into_money, (' . db_prefix() . 'pur_order_detail.total-' . db_prefix() . 'pur_order_detail.into_money) as tax_money, total as goods_money, wh_quantity_received, ' . db_prefix() . 'pur_order_detail.discount_money as discount_money from ' . db_prefix() . 'pur_order_detail
		...
		foreach ($results as $key => $value) {
			...
			if((float)$value['quantities'] - (float)$value['wh_quantity_received'] > 0){
				$discount_per = ($value['discount_money']/$value['into_money']) * 100;
				$discount_per = number_format((float)$discount_per, 2, '.', '');
				$total_amount = $value['into_money'] - $value['discount_money'];
				...
				$arr_results[$index]['discount'] = $discount_per."%";
				$arr_results[$index]['discount_money'] = $value['discount_money'];
				$arr_results[$index]['total_amount'] = $total_amount;
		...
		$total_money = ($total_goods_money + $total_tax_money) - $value['discount_money'];
		...
		$arr_pur_resquest[] = $discount_money;
    	...
    	if(isset($hot_purchase)){
    		...
    		foreach ($goods_delivery_detail as $key => $value) {
    			...
    			if($value[0] != '' && isset($value[13])){
    				...
    				$this->db->where('goods_delivery_id', $goods_delivery_id);
    				$this->db->update(db_prefix() . 'goods_delivery_detail', ['quantities' => $value[4], 'unit_price' => $value[5], 'total_money' => $value[7], 'note' => $value[12]]);
