...
class warehouse extends AdminController {
	...
	public function coppy_pur_request($pur_request) {
		...
		echo json_encode([
			...
			'discount_money' => $pur_request_detail[5] ? $pur_request_detail[5] : '',
			'total_row' => $pur_request_detail[6] ? $pur_request_detail[6] : '',
		]);
	}
	...
	public function commodity_list_add_edit($id = '') {
		...
		if ($data) {
			...
			if (!isset($data['id'])) {
				...
				$id = $data['id'];
				unset($data['id']);
				$success = $this->warehouse_model->update_commoadity_one_item($data, $id);
