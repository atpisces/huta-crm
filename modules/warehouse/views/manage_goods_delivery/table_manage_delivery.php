...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
    ...
    $num_rows = $CI->invoices_model->check_invoice($aRow["id"]);
    $convert_status = ''; $to_invoice = '';

    if($num_rows > 0){
        $convert_status = '<span class="label label-tag tag-id-1 label-tab1"><span class="tag">CONVERTED</span></span>';
        $to_invoice = '<span class="label label-tag tag-id-1 label-tab1"><span class="tag">CONVERTED</span></span>';
    } else {
        $convert_status = '<a href="'.base_url('admin/invoices/invoice?d_note='.$aRow["id"]).'" class="btn btn-sm btn-success btn-icon" target="_blank">Convert to Invoice</a>';
        $to_invoice = '<div class="checkbox checkbox-primary">
                            <input type="checkbox" name="conver_to_invoice" id="conver_to_invoice" value="'.$aRow["id"].'">
                            <label for="conver_to_invoice"></label>
                        </div>';
    }
