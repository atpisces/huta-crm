...
class Accounting extends AdminController
{
     ...
     * @return view
     */
    public function rp_hr_staff_list(){
        $this->load->model('currencies_model');
        $data['title'] = 'Staff Members List';
        $data['from_date'] = date('Y-01-01');
        $data['to_date'] = date('Y-m-d');
        $data['currency'] = $this->currencies_model->get_base_currency();
        $this->load->view('report/includes/hr_staff_list', $data);
    }
    ...
    public function view_report(){
        ...
        switch ($data_filter['type']) {
            ...
            case 'profit_and_loss_budget_vs_actual':
                ...
                break;
            case 'hr_staff_list':
                    // echo "Here: ".$data_filter['type'];
                    $data['from_date'] = date('Y-m-01');
                    $data['to_date'] = date('Y-m-d');
                    $data['data_report'] = $this->accounting_model->get_staff_list($data_filter);
