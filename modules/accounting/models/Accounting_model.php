...
class Accounting_model extends App_Model
{
    ...
    public function get_staff_list($data_filter = '', $staffid = ''){
        if($staffid == ''){
            return $this->db->query("SELECT * FROM tblstaff S LEFT JOIN tblroles R ON R.roleid = S.role ORDER BY S.staffid ASC")->result_array();
        } else {
