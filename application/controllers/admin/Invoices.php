...
class Invoices extends AdminController
{
	...
	// 	$customer = "";
	// 	//if($customer != ""){
			// $delivery_notes = $this->invoices_model->get_all_delivery_notes($customer);
	// 		$options = '';
	// 		foreach($delivery_notes as $note){
	// 			$options .= '<option value="">Select Delivery Note</option><option value="'.$note['id'].'">'.$note['goods_delivery_code'].'</option>';
	// 		}
	// 		echo $options;
	// 	//}
	// }
    ...
    public function getDeliveryNoteItems(){
        ...
        $delivery_note = $this->input->post("note");

        $data['taxes'] = $this->taxes_model->get();
        $default_tax = unserialize(get_option('default_tax'));
        $select_delivery_notes = '<select class="selectpicker display-block tax main-tax" data-width="100%" name="taxname" multiple data-none-selected-text="'._l('no_tax').'">';
      //  $select_delivery_notes .= '<option value=""'.(count($default_tax) == 0 ? ' selected' : '').'>'._l('no_tax').'</option>';
        ...
        $select_delivery_notes .= '<option value="'.$tax['name'].'|'.$tax['taxrate'].'"'.$selected.'data-taxrate="'.$tax['taxrate'].'" data-taxname="'.$tax['name'].'" data-subtext="'.$tax['name'].'">'.$tax['taxrate'].'%</option>';
        }
        $select_delivery_notes .= '</select>';
        // echo $select_delivery_notes;
        $items_data = '';
        $customer = '';
        // echo "Delivery Note Selectin: ".$delivery_note;
        $delivery_notes_list = $this->invoices_model->get_delivery_note_items($delivery_note);
        // print_r($delivery_notes_list);
        // echo "Delivery note is: ".$delivery_note;

        $items_data = ''; $inc = 1;
        foreach($delivery_notes_list as $note){
            $items_data .= '<tr class="main"><td></td><td><textarea name="description" class="form-control" rows="4" placeholder="Description">'.$note["description"].'</textarea></td><td><textarea name="long_description" rows="4" class="form-control" placeholder="Long description">'.$note["long_description"].'</textarea></td><td><input type="number" name="quantity" min="0" class="form-control" placeholder="Quantity" value="'.$note["quantities"].'"><input type="text" placeholder="Unit" data-toggle="tooltip" data-title="e.q kg, lots, packs" name="unit" class="form-control input-transparent text-right" value="'.$note["unit_name"].'"></td><td><input type="number" name="rate" class="form-control" placeholder="Rate" value="'.$note["unit_price"].'"></td><td align="right">'.$note["tax_name"].' '.$note["taxrate"].' '.$select_delivery_notes.'</td><td>'.$note["total_money"].'</td><td><button type="button" onclick="add_item_to_table(\'undefined\',\'undefined\',undefined,\''.$inc.'\'); return false;" class="btn pull-right btn-info"><i class="fa fa-check"></i></button></td></tr>';
            $inc++;
        }
        echo $items_data;
    }

    /* Individual Invoice Import */
    public function import_individual_csv()
    {
        $this->load->view('admin/invoices/invoice_import');
    }


    /* Bulk Invoices Import */
    public function import_bulk_csv()
    {
        $this->load->view('admin/invoices/invoice_import_bulk');
    }
	...
	public function delete_invoice($id = '')
	{
		...
		{
			set_alert('success', _l('Invoice deleted successfully.', _l('invoice')));
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            $delivery_notes = implode(",",$invoice_data['delivery_note']);
            unset($invoice_data['delivery_note']);
            // echo "<br><br><br>";
            // echo $delivery_notes;
            $invoice_data['delivery_note'] = $delivery_notes;
            // print_r($invoice_data);
            // exit;
            if ($id == '') {
                if (!has_permission('invoices', '', 'create')) {
        ...
        $customer = "";
        $data['delivery_notes'] = $this->invoices_model->get_all_delivery_notes($customer);
