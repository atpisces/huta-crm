...
$info_left_column1 .= $invoice->recovery_payment.''.$invoice->recovery_label_field;
                  ...
                  if($invoice->recovery_label_field == "%"){
                    $recovery_c_vat = $amt_exc_vat * ($invoice->recovery_payment/100);
                    $recovery_vat = $recovery_c_vat * (15/100);
                    $recovery_with_vat = $recovery_c_vat + $recovery_vat;
                  } else {
                    $recovery_c_vat = $invoice->recovery_payment;
                    $recovery_vat = $recovery_c_vat * (15/100);
                    $recovery_with_vat = $recovery_c_vat + $recovery_vat;
                  }
$info_left_column1 .= '<br> استرداد الدفعة المقدمة</td>
                     <td align="center" class="invoice-table-column" style="border: 1px solid #000000;">'.app_format_money($recovery_c_vat, $invoice->currency_name).'</td>
                     <td align="center" class="invoice-table-column" style="border: 1px solid #000000;">15%</td>';
$info_left_column1 .= '<td width="12%" align="center" class="invoice-table-column" style="border: 1px solid #000000;">'.app_format_money($recovery_vat, $invoice->currency_name).'</td>
                  ...
                  $rem_amt_exc = $amt_exc_vat - $recovery_c_vat;
                  $rem_amt_vat = $totalvat - $recovery_vat;
                  $rem_amt_total = $totalsar - $recovery_with_vat;
...
$info_left_column1 .= '<tr style="background: #f2f2f2; color: #000000">
  ...
  //  }
                  $withheld_amt = 0;
                  if($invoice->withheld_label_field == "%"){
                    $withheld_amt = $rem_amt_total * ($invoice->withheld_payment/100);
                  } else {
                    $withheld_amt = $invoice->withheld_payment;
                  }
                  $net_amt_due = $rem_amt_total - $withheld_amt;
                  $net_amt_due = $net_amt_due - $invoice->other_deductions_field;
$info_left_column1 .= '<tr>
                     <td colspan="5" class="invoice-table-column" style="border: 1px solid #000000;">Amount withheld as Retention -'.$invoice->withheld_payment.''.$invoice->withheld_label_field.' <br> خصم الاستبقاء</td>
                     <th align="right" class="invoice-table-column" style="border: 1px solid #000000;">('.app_format_money($withheld_amt, $invoice->currency_name).')</th>
                  </tr>
                  <tr>
                     <td colspan="5" class="invoice-table-column" style="border: 1px solid #000000;">Other Deductions <br> استقطاعات أخرى</td>
                     <th align="right" class="invoice-table-column" style="border: 1px solid #000000;">('.app_format_money($invoice->other_deductions_field, $invoice->currency_name).')</th>
                  </tr>
                  <tr>
