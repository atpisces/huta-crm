         ...
         <div class="col-md-6">
            <div class="f_client_id">
              <div class="form-group select-placeholder">
               ...
               <?php 
                // if(isset($_REQUEST["d_note"])){
                //   $CI =& get_instance();
                //   $CI->load->model('invoices_model');
                //   $delivery_note_details = $CI->invoices_model->get_delivery_note_details($_REQUEST["d_note"]);
                //   echo $delivery_note_details[0]["customer_code"];
                // }
                //   print_r($delivery_note_details);
               ?>
                <select id="clientid" name="clientid" data-live-search="true" data-width="100%" class="ajax-search<?php if(isset($invoice) && empty($invoice->clientid)){echo ' customer-removed';} ?>" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>" onchange="selectCustomerForInvoice(this.value)">
               <?php 
                $selected = '';
                if(isset($_REQUEST["d_note"])){
                  $CI =& get_instance();
                  $CI->load->model('invoices_model');
                  $delivery_note_details = $CI->invoices_model->get_delivery_note_details($_REQUEST["d_note"]);
                  $selected = $delivery_note_details[0]["customer_code"];
                } else {
            ...
            <?php } //print_r($delivery_notes); ?>
			 
			 <div class="form-group mbot15">
				 <label for="delivery_notes_list" class="control-label">Delivery Notes</label>
				 <br />
				 <select class="form-control" name="delivery_note[]" id="delivery_notes_list" onchange="getDeliveryNoteItems(this.value)" multiple style="height: 200px;">
             <option value="">Select Delivery Note</option>
         <?php
            foreach($delivery_notes as $note){
                echo '<option value="'.$note['id'].'">'.$note['goods_delivery_code'].'</option>';
            }
         ?>
				 </select>
			 </div>
               ...
               <tr class="main">
                  ...
                  <td>
                     <?php
                        ...
                        ?>
                     <button type="button" onclick="add_item_to_table('undefined','undefined',<?php echo $new_item; ?>,'0'); return false;" class="btn pull-right btn-info"><i class="fa fa-check"></i></button>
                  </td>
               </tr>
