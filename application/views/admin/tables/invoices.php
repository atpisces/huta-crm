...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    db_prefix() . 'invoices.delivery_note',
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '';
    $converted = '';
    if($aRow["delivery_note"] != ""){
        $converted = '<a href="javascript:void(0)" class="btn btn-sm btn-success btn-icon">Converted from Credit Note</a>';
    }
    // If is from client area table
    if (is_numeric($clientid) || $project_id) {
        $numberOutput = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" target="_blank">' . format_invoice_number($aRow['id']) . '</a>'.$converted;
    } else {
        $numberOutput = '<a href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" onclick="init_invoice(' . $aRow['id'] . '); return false;">' . format_invoice_number($aRow['id']) . '</a>'.$converted;
    }
    ...
    if (has_permission('invoices', '', 'edit')) {
		if($aRow['number'] == "1000000000"){
        	$numberOutput .= ' | <a href="' . admin_url('invoices/invoice/' . $aRow['id']) . '">' . _l('edit') . '</a>';
		}
    }
    if (has_permission('invoices', '', 'delete')) {
		if($aRow['number'] == "1000000000"){
        	$numberOutput .= ' | <a onclick="return confirm(\'Are you sure? You want to delete this record?\');" href="' . admin_url('invoices/delete_invoice/' . $aRow['id']) . '" class="text-danger">' . _l('Delete') . '</a>';
		}
    }
    $numberOutput .= '</div>';
