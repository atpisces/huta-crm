...
class Invoices_model extends App_Model
{
	...
	public function get_all_delivery_notes($customer){
		if($customer != ""){
            return $this->db->query("SELECT * FROM " . db_prefix() . "goods_delivery WHERE customer_code = '".$customer."'")->result_array();
		} else {
			return $this->db->query("SELECT * FROM " . db_prefix() . "goods_delivery")->result_array();
		}
	}
    
    public function get_delivery_note_items($delivery_note){
        
        return $this->db->query("SELECT *,T.name as tax_name FROM " . db_prefix() . "goods_delivery D INNER JOIN " . db_prefix() . "goods_delivery_detail G ON G.goods_delivery_id = D.id INNER JOIN " . db_prefix() . "items I ON I.id = G.commodity_code LEFT JOIN " . db_prefix() . "ware_unit_type U ON U.unit_type_id = G.unit_id LEFT JOIN " . db_prefix() . "taxes T ON T.id = G.tax_id  WHERE FIND_IN_SET(D.id,'".$delivery_note."')")->result_array();
    }

    public function get_delivery_note_details($delivery_note){
        return $this->db->query("SELECT * FROM " . db_prefix() . "goods_delivery WHERE id = '".$delivery_note."'")->result_array();
    }

    public function check_invoice($delivery_note){
        // $this->db->where('delivery_note', $delivery_note);
        // $num_rows = $this->db->get(db_prefix() . 'invoices')->num_rows();
        // return $num_rows;
        return $this->db->query("SELECT * FROM " . db_prefix() . "invoices WHERE FIND_IN_SET('".$delivery_note."',delivery_note)")->num_rows();
    ...
    public function get_invoice_item_by_type($id)
    {
