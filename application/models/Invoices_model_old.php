...
class Invoices_model extends App_Model
{
    ...
    public function get_invoice_payment_modes($ids)
    {
        // $this->db->where('id', $id);
        ...
        return $this->db->query("SELECT * FROM " . db_prefix() . "payment_modes WHERE find_in_set(id,'".$ids."')")->result_array();
    }

    public function get_extra_fields_data($id,$itype)
    {

        return $this->db->query("SELECT * FROM " . db_prefix() . "invoice_extrafields WHERE invoiceid = '".$id."' AND invoicetype = '".$itype."'")->result_array();
    ...
    public function get_project_details($id){
        if($id){
            ...
            $q = $this->db->get(db_prefix() . 'projects')->result_array();
            return $q;
        }
    ...
    public function add($data, $expense = false)
    {
        ...
        // print_r($items);
        // exit;
        $extrafields = $data['extra_fields_type'];
        $extradescription = $data['extra_fields_description'];
        $extraamounts = $data['extra_fields_amount'];
        $extravats = $data['extra_fields_vat'];
        unset($data['extra_fields_type']);
        unset($data['extra_fields_description']);
        unset($data['extra_fields_amount']);
        unset($data['extra_fields_vat']);
        //print_r($extrafields);
        //echo "<br>";
        //print_r($extraamounts);
        //echo "<br>";
        //print_r($extravats);
        //echo "<br>";
        ...
        if ($insert_id) {
            ...
            if(isset($data['project_id'])){
                if($data['project_id'] != ""){
                    $project_details = $this->get_project_details($data['project_id']);
                    $project_code = "";
                    $nextinvoice = "";
                    $pdf_custom_fields = get_custom_fields('projects', array('show_on_pdf' => 0, 'show_on_client_portal' => 1));
                    // print_r($pdf_custom_fields);

                    $nextinvoice = sprintf("%03d", $insert_id);

                    foreach ($pdf_custom_fields as $field) {
                        $value = get_custom_field_value($data['project_id'], $field['id'], 'projects');
                        if ($value == '') {
                          continue;
                        }
                        if($field['name'] == "Project Code"){
                            $project_code = $value;
                        }
                    }
                    // exit;
                    $new_invoice_no = "HH-".$project_code."-".date("Y")."-".$nextinvoice;
            ...
            for($i=0;$i<count($extrafields);$i++){
                if($extrafields[$i] != ""){
                    //$extra_fields_data = array("invoiceid" => $insert_id, "fieldtype" => $extrafields[$i], "fieldamount" => $extraamounts[$i], "fieldvat" => $extravats[$i]);
                    $extra_fields_data = array("invoiceid" => $insert_id,"invoicetype" => "invoice", "fieldtype" => $extrafields[$i], "fielddescription" => $extradescription[$i], "fieldamount" => $extraamounts[$i], "fieldvat" => $extravats[$i]);
                    // echo $extrafields[$i]." ".$extraamounts[$i]." ".$extravats[$i]."<br>";
                    // print_r($extra_fields_data);
                    $this->db->insert(db_prefix() . 'invoice_extrafields', $extra_fields_data);
                }
            }
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('invoicetype', "credit_note");
        $this->db->delete(db_prefix() . 'invoice_extrafields');
