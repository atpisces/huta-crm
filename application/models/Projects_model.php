...
class Projects_model extends App_Model
{
    ...
    public function get_all_projects($clientid = ''){
        if($clientid){
            $this->db->where(db_prefix() . 'projects.clientid', $clientid);
    ...
    {
        $this->db->where("id",$id);
        $data = $this->db->get(db_prefix() . 'projects')->row();
        return $data;
    }
    ...
    //     $data = $this->db->get(db_prefix() . 'files')->result_array();
    //     return $data;
    // }

    public function get_tickets($project_id)
    {
        $this->db->select('*');
        $this->db->where('project_id', $project_id);
        $this->db->order_by('ticketid', 'ASC');
        $data = $this->db->get(db_prefix() . 'tickets')->result_array();
        return $data;
    }

    public function get_contracts($project_id)
    {
        $this->db->select('*');
        $this->db->where('project_id', $project_id);
        $this->db->order_by('id', 'ASC');
