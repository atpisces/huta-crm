...
function initDataTable(e, t, a, i, n, s) {
    var table_order = "asc"; var table_index = 0; if(e == ".table-notes"){ table_order = "desc"; table_index = 4; }
...
function add_item_to_table(n, s, e, t) {
	// alert($('.main').length);
    // alert(n+" - "+s+" - "+e+" - "+t+" - ");
    var unique_note_index = t;
    if ("" !== (n = void 0 === n || "undefined" == n ? get_item_preview_values(unique_note_index) : n).description || "" !== n.long_description || "" !== n.rate) {
    // alert($('.main').length);
        var p = "",
...
function get_item_preview_values(u) {
    // alert("Getting: "+u);
    var e = {};
    return e.description = $('.main textarea[name="description"]').eq(u).val(), e.long_description = $('.main textarea[name="long_description"]').eq(u).val(), e.qty = $('.main input[name="quantity"]').eq(u).val(), e.taxname = $(".main select.tax").eq(u).selectpicker("val"), e.rate = $('.main input[name="rate"]').eq(u).val(), e.unit = $('.main input[name="unit"]').eq(u).val(), e
}
...
function selectCustomerForInvoice(customer){
	//alert(customer);
	/*$.post(admin_url + 'invoices/delivery_notes_list?customer='+customer).done(function(response) {
		//alert("Response: "+response);
		$("#delivery_notes_list").append(response);
	});*/
}
// alert('');
// $.ajax({
//     url: admin_url + 'invoices/delivery_notes_list',
//     type: "post",
//     success: function (response) {
//         alert("Delivery Notes Response: "+response);
//         $("#delivery_notes_list").html(response);
//     },
...
function getDeliveryNoteItems(note){
    if(note == ""){
        $(".main").eq(0).show();
        ...
        $(".main").not(':first').remove();
        $.ajax({
            url: admin_url + 'invoices/getDeliveryNoteItems',
            type: "post",
            data: {note : note},
            success: function (response) {
                // alert("Delivery Items Response: "+response);
                $("#alllineitems").append(response);
            },
            error: function(jqXHR, textStatus, errorThrown) {
               console.log(textStatus, errorThrown);
            }
...
function getSelectedDeliveryNotes(){
    var deliveryNotes = new Array();
    $("input:checkbox[name=conver_to_invoice]:checked").each(function(){
        deliveryNotes.push($(this).val());
    });
    if(deliveryNotes != ""){
        var move_to = $("#system_base_url").val();
        // alert(move_to+""+deliveryNotes);
        // window.location.href = move_to+""+deliveryNotes;
        window.open(move_to+""+deliveryNotes, '_blank');
    } else {
        alert("Please select at least one delivery note to convert it into invoice.");
        $("#conver_to_invoice").first().focus();
    }
}
...
var d_note = getUrlParameter('d_note');
if(d_note){
    // alert(d_note);
    // $("#delivery_notes_list").val(d_note);
    var d_note_array = JSON.parse("["+d_note+"]");
    $("#delivery_notes_list").val(d_note_array);
    setTimeout(function(){ 
        getDeliveryNoteItems(d_note);
        // alert("getting");
    },1000);
}

function extendDate(staffid, statustype){
    if(statustype == "iqama"){
        $("#expirymodaltitle").text("Extend Iqama Date");
    } else if(statustype == "passport"){
        $("#expirymodaltitle").text("Extend Passport Date");
    } else if(statustype == "medical"){
        $("#expirymodaltitle").text("Extend Medical Insurance Date");
    }
    $("#years_extend_duration").val("0");
    $("#months_extend_duration").val("0");
    $("#days_extend_duration").val("0");
    // alert(statustype);
    $("#staffid").val(staffid);
